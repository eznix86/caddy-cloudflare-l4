...
# Caddy Cloudflare L4
> A fully integrated Caddy Docker image featuring Cloudflare DNS-01 ACME validation and Layer 4 (TCP/UDP) proxying

Deploy a hassle-free Caddy server with built-in support for Cloudflare DNS-01 ACME challenges and Layer 4 proxy capabilities. Streamline your SSL certificate management, proxy TCP/UDP traffic, and ensure your server stays secure without manual updates.
...
- **Layer 4 Proxy**: Includes [caddy-l4](https://github.com/mholt/caddy-l4) module for TCP/UDP proxying, enabling non-HTTP protocol handling.
...
Replace `eznix86` with your GitHub username and `your_cloudflare_api_token` with the actual token you generated.
...
The image on GitHub Container Registry provides the following tags:

- **`latest`**:
  - Always points to the most recent stable release of Caddy with Cloudflare DNS and Layer 4 modules.
...
- **`<version>`**:
  ...
  - Examples include:
    - **`2.8.0`**: Full version tag for Caddy version 2.8.0, ensuring you are using this exact release.
    - **`2.8`**: Minor version tag for the latest patch release within the 2.8 series.
    - **`2`**: Major version tag for the latest release within the 2.x series.

- `alpine`: Always points to the latest stable release of the Alpine-based image.
  - `<version>-alpine`: Specific version tags for the Alpine-based image (e.g., `2.8.0-alpine`).

## Platform Support

The image is built to support multiple platforms, ensuring compatibility across a wide range of devices and systems. The supported platforms include:
...
To use the Alpine-based image, pull it from the GitHub Container Registry:
...
This Docker image is optimized for Raspberry Pi, allowing you to deploy Caddy with Cloudflare DNS integration and Layer 4 proxying on these popular single-board computers. Whether you are using a Raspberry Pi 3 or the latest Raspberry Pi 4, this image provides the necessary support for seamless operation.
...
Replace `eznix86` with your GitHub username and `your_cloudflare_api_token` with your actual Cloudflare API token.
